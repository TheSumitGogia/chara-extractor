...
class Book:
    ...
    def writeToFile(self, outdir='sparknotes/descriptions'):
        if len(self.characters) == 0:
            return
        directory = (outdir + '/%s') % self.name
        ...
        with open(outdir + '/combined/%s' % (self.name), 'w') as f:
...
book.writeToFile()
'''
def collect(outdir='sparknotes/descriptions'):
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    if not os.path.exists(outdir + '/combined'):
        os.makedirs(outdir + '/combined')
    ...
    for book in exclude:
        ...
        call(["rm", "-rf", outdir+"/"+book])
    booklist = open(outdir + '/../booklist.txt', 'w')
    towrite = [book for book in books if book not in exclude]
    booklist.write('\n'.joini(towrite))
    booklist.close()
