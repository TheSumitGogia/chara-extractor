...
def process(book, tagdir='annotations'):
    ...
    if writeToFile:
        with open(write_dir + '/%s_characters.txt' % book.name, 'w') as f:
            characters = dict([(c, book.characters[c].references) for c in book.characters])
            f.write(str(characters))
        with open(write_dir + '/%s_relations.txt' % book.name, 'w') as f:
            f.write(str(rel_pred))

    if compare:
        with open(tagdir + '/%s.tag'%book.name) as f:
            while True:
                line=f.readline()
                if line== "":
                    break
...
def test(bookdir, tagdir):
    recall = 0
    precision = 0
    annotated = filter(lambda fname: fname.endswith("tag"), os.listdir(tagdir))
    annotated = [name[:len(name)-4] for name in annotated]
    unsatisfied = []
    books_dir = bookdir
    for book in annotated:
        (r, p) = process(book, tagdir)
        if r < 0.7 or p < 0.7:
...
def process_all(sndir, bookdir, outdir):
    compare = verbose
    books_dir = sndir
    writeToFile = True
    raw = filter(lambda fname: fname.endswith("txt"), os.listdir(bookdir))
