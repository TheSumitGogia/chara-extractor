...
stanford_dir = os.path.dirname(os.path.realpath(__file__)) + '/stanford'
def populate_gender_dict():
  d = {}
  with open(stanford_dir + '/namegender.combine.txt') as f:
    for line in f:
      name, gender = line.rsplit('\t', 1)
      d[name.strip()] = gender.strip()
  with open(stanford_dir + '/male.unigrams.txt') as f:
    for name in f:
      d[name.strip()] = 'MALE'
  with open(stanford_dir + '/female.unigrams.txt') as f:
    for name in f:
      d[name.strip()] = 'FEMALE'
  return d
